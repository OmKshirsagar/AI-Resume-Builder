				...
				return (
					experience.length > 0 && (
						<View key="experience" style={tw("mb-4")}>
							<Text
								style={tw(
									"border-b border-slate-900 font-bold uppercase text-[9px] mb-2 pb-0.5",
								)}
							>
								Experience
							</Text>
							{experience.map((exp) => (
								<View key={exp.id} style={tw("mb-3")}>
									<View style={tw("flex-row justify-between")}>
										<Text style={tw("font-bold text-[10px] text-slate-900")}>
											{exp.company}
										</Text>
										<Text style={tw("text-[9px] text-slate-600")}>
									...
									<View style={tw("flex-row justify-between mb-1")}>
										...
										<Text style={tw("text-[9px] text-slate-700")}>
											{exp.location}
										</Text>
									</View>
				...
				return (
					skills.length > 0 && (
						<View key="skills" style={tw("mb-3")}>
							<Text
								style={tw(
									"border-b border-slate-900 font-bold uppercase text-[9px] mb-1.5 pb-0.5",
								)}
							>
								Skills
							</Text>
								...
								<View>
									{customSection.items.map((item) => (
										<View key={item.id} style={tw("mb-2")}>
											<View style={tw("flex-row justify-between mb-0.5")}>
												<Text
													style={tw("font-bold text-[10px] text-slate-900")}
												>
													{item.title}
												</Text>
												<Text style={tw("text-[9px] text-slate-600")}>
	...
	return (
		<Document>
			...
			>
				...
				<View
					style={tw(
						"flex flex-col items-center mb-4 pb-2 border-b border-slate-100",
					)}
				>
					...
					<View
						style={tw(
							"flex-row justify-center gap-3 text-[9px] text-slate-600 font-bold uppercase",
						)}
					>
				...
				{personalInfo?.summary && (
					<View style={tw("mb-5 px-10")}>
						<Text
							style={tw(
								"text-[10px] text-center italic text-slate-700 font-medium",
							)}
						>
							{personalInfo.summary}
						</Text>
